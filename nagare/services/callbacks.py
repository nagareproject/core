...
class CallbacksService(plugin.Plugin):
    ...
    def handle_request(cls, chain, callbacks, request, response, root, **params):
        ...
        for (callback_type, callback_id, param, client_params), values in sorted(actions.items()):
            ...
            # 3 : <form>.post_action
            # 4 : action with continuation and without value (<a>, ...)
            # 5 : action with continuation and without value (submit button)
            ...
            if callback_type == '1':
                args += (values[0] if len(values) == 1 else tuple(values),)
                f(*args, **kw)
